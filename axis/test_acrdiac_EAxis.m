...
%             disp(ii);
    ...
    mean_diff_P = mean(cmp_PAxis(   ~isnan(cmp_PAxis(:,3)),3));
    std_P = std(cmp_PAxis(:,3));
    mean_diff_R = mean(cmp_RAxis(:,3));
    std_R = std(cmp_RAxis(:,3));
    mean_diff_T = mean(cmp_TAxis(:,3));
    std_T = std(cmp_TAxis(:,3));

    res= meas_qt(cmp_PAxis(:,1),cmp_PAxis(:,2));
     res= meas_qt(cmp_RAxis(:,1),cmp_RAxis(:,2));
      res= meas_qt(cmp_TAxis(:,1),cmp_TAxis(:,2));
    subplot(3,1,1)
